...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        Lettre newLettre(char caractere, boolean visible){
            Lettre lettre = new Lettre();
            lettre.caractere = caractere;
            lettre.visible = visible;
            return lettre;
        }
        ...
        Bonus[] initialiserBonus(Bonus[] listebonus){
            for (int i=0;i<3;i++){
                Bonus b = new Bonus();
                listebonus[i]=b;
            }
            listebonus[0].nom="Vision";
            listebonus[1].nom="Vie + 1";
            listebonus[2].nom="Skip";
            return listebonus;
        }

        Bonus bonusaleatoire (Utilisateur user, Bonus[] listebonus){
            Bonus b = listebonus[(int)(random() * 3)];

            for(int i = 0; i < length(user.listebonus); i++){
                if(user.listebonus[i] != null && equals(user.listebonus[i].nom, b.nom)){
                    b.nombre++;
                }
            }   
        ...
        void utiliserbonus(Utilisateur user, String reponse, Mot motactuel, String langue, Mot[] words){
            int indice = charAt(reponse,0) - '0' - 3;
            if(user.listebonus[indice].nom=="Vision"){
                String motATraduire = retournerMotATraduire(motactuel, langue);
                int moitieMot = length(motATraduire)/2;
                while(moitieMot != 0){
                        int random = (int)(random() * length(motATraduire));
                        if(equals(langue,"français")){
                            moitieMot--;
                        }else if(equals(langue,"anglais")){
                            moitieMot--;
                        }
                    }
                }
                enleverbonus(user, user.listebonus[indice]);

            
            if (equals(user.listebonus[indice].nom,"Vie + 1")){
                user.premierjeu_vies++;
                enleverbonus(user, user.listebonus[indice]);
            }
            if (equals(user.listebonus[indice].nom,"Skip")){
        ...
        void enleverbonus(Utilisateur user, Bonus bonus){
            for(int i = 0; i < length(user.listebonus); i++){
                if(equals(user.listebonus[i].nom,bonus.nom)){
                    user.listebonus[i].nombre--;
                    if (user.listebonus[i].nombre != 0){
                        user.listebonus[i] = null;
                    }
                }
            }
        }
        ...
        Utilisateur Traduction(Mot[] words,Utilisateur user){
            Bonus[] listebonus = new Bonus[3];
            listebonus = initialiserBonus(listebonus);
            ...
            while(recommencer){
                ...
                while (user.premierjeu_vies>0 && !(user.premierjeu_score==5) && !quitter){
                    ...
                    motactuel = words[(int)(random()*length(words,1))];
                    GUI = MAJ_AvanceJeuTraduction(GUI, langue, retournerMotATraduire(motactuel, langue),user);
                    affichage(GUI);
                    println();
                    ...
                    if (bonresultat(reponse,motactuel,langue)){
                        ...
                        if (bonusaleatoire(user,listebonus).nombre == 0){
                            user.listebonus[indice]=bonusaleatoire(user,listebonus);
                        }
                        else {
                            user.listebonus[position].nombre++;
                        }
                        indice = indice + 1;
                        println(user.listebonus[0].nom);
                        println(user.listebonus[0].nombre);
                        user.premierjeu_mots_traduits++;
                    }
                    else if(equals(reponse,"2")){;
                        quitter=true; 
                    }
    ...
    String integrerBonusJeuTraduction(String ligne, String nombonus){
        int position = chercheEmplacementReperes(ligne, '-', 1)[0]+1;
        String partieGauche = substring(ligne, 0, position+1);
        String espace = substring(ligne, position+1, position+1 + length(nombonus));
        String partieDroite = substring(ligne, position+1 + length(nombonus), length(ligne));
        return (partieGauche + completerVide(espace, nombonus, 0) + partieDroite);
    }

    String[] MAJ_AvanceJeuTraduction(String[] GUI, String langue, String mot, Utilisateur user){
        GUI[8] = insererLangueJeuTraduction(GUI[8], langue);
        GUI[11] = insererMotJeuTraduction(GUI[11], mot);
        String txt="";
        if(user.listebonus[0] != null) txt = user.listebonus[0].nom + " x" + user.listebonus[0].nombre;
        GUI[7] = integrerBonusJeuTraduction(GUI[7], txt);
        txt = "";
        if(user.listebonus[1] != null) txt = user.listebonus[1].nom + " x" + user.listebonus[1].nombre;
        else txt = "";
        GUI[9] = integrerBonusJeuTraduction(GUI[9], txt); 
        if(user.listebonus[2] != null) txt = user.listebonus[2].nom + " x" + user.listebonus[2].nombre;
        else txt = "";
        GUI[11] = integrerBonusJeuTraduction(GUI[11], txt);

        for(int i = 0; i < length(user.listebonus); i++){
            if(user.listebonus[i] != null){
                println(i + ". " + user.listebonus[i].nom);
            }else{
                println(i + ". vide");
            }
        }
